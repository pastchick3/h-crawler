...
pub fn crawl_galleries(
    ...
    for gallery in galleries {
        ...
        let fill = if x { "x" } else { "-" };
        let id = format!("{}/{}/", parts[0], parts[1]);
        let url = format!("https://e{fill}hentai.org/g/{id}");
        let range = parts[2];
        ...
        // Check if this gallery is not available in E-Hentai.
        if !x && document.select_first("#continue").is_ok() {
            println!("Gallery {id} is not available. Try to use ExHentai.");
            continue;
        }
